...
class LinearRegressionMAPEstimator():
    ...
    def fit(self, x_ND, t_N):
        ...
        I = np.eye(M) #D
        first_term = np.matmul(np.transpose(Phi_NM),Phi_NM)
        second_term = (self.alpha/self.beta)*I
        self.w_map_M = np.matmul(np.linalg.inv(first_term + second_term),np.matmul(np.transpose(Phi_NM),t_N))
        return self
    ...
    def predict(self, x_ND):
        ...
        for i in range(N):
            mean = np.matmul(np.transpose(self.w_map_M),phi_NM[i])
            #variance = 1/self.beta
            #prediction[i] = scipy.stats.norm.rvs(loc=mean,scale=variance,size=1)[0]
            prediction[i] = mean
