...
if __name__ == '__main__':
    ...
    for dataset_name in ['toyline', 'toywave']:
        ...
        for order, fig1_map_ax, fig2_ppe_ax in zip(
            ...
            for ii, N in enumerate(n_train_list):  # id val
                ...
                map_train_scores[ii]= map_estimator.score(x_train_ND[:N],t_train_N[:N])
                ## TODO record estimator's score on test in map_test_scores
                map_test_scores[ii] = map_estimator.score(x_test_ND[:N],t_test_N[:N])
                print("%6d examples : train score % 9.3f | test score % 9.3f" % (
                    N, map_train_scores[ii], map_test_scores[ii]))

                # Obtain predicted mean
                #mean_G = np.zeros(G) # TODO call predict to get map_estimator's
                mean_G = map_estimator.predict(x_train_ND[:G])

                # Obtain predicted standard deviation (sqrt(variance))
                #stddev_G = np.ones(G)
                stddev_G = np.ones(G)* math.sqrt(1/beta) # TODO fixme (Hint: depends on beta)
            ...
            for ii, N in enumerate(n_train_list):
                ...
                ppe_train_scores[ii]=ppe_estimator.score(x_train_ND[:N], t_train_N[:N])
                ## TODO record estimator's score on test in ppe_test_scores
                ppe_test_scores[ii] = ppe_estimator.score(x_test_ND[:N], t_test_N[:N])
                print("%6d examples : train score % 9.3f | test score % 9.3f" % (
                    N, ppe_train_scores[ii], ppe_test_scores[ii]))

                # Obtain predicted mean
                #mean_G = np.zeros(G) # TODO call predict to get mean
                mean_G = ppe_estimator.predict(x_train_ND[:G])
                # Obtain predicted standard deviation (sqrt(variance)) 
                #stddev_G = np.ones(G) # TODO call predict_variance
                stddev_G = np.sqrt(ppe_estimator.predict_variance(x_train_ND[:G]))
