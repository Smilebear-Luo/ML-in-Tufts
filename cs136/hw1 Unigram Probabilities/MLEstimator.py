...
class MLEstimator():
    ...
    * 2) does not give zero probability to any word
    ...
    def fit(self, word_list):
        ...
        * The integer total_count is set to the total length of the word list
        ...
        for i in range(len(word_list)):
            id = self.vocab.get_word_id(word_list[i])
            self.count_V[id] = self.count_V[id] + 1

        for j in range(self.count_V.size):
            if self.count_V[j]==0:
                self.unseen_count = self.unseen_count+1
    def predict_proba(self, word):
        ...
        nv = self.count_V[id]
        if nv==0:
            return 1/self.unseen_count * self.unseen_proba
        else:
            return (1 - self.unseen_proba) * (nv / self.total_count)
