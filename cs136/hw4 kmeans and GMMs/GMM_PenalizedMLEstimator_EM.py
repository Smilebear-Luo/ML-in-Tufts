...
class GMM_PenalizedMLEstimator_EM(GMM_PenalizedMLEstimator_LBFGS):
    ...
    * Initialization method generate_initial_parameters() from parent as well
    ...
    def calc_EM_loss(self, r_NK, x_ND):
        ...
        * Penalty on the standard deviation parameters
        ...
        for k in range(self.K):
            first = np.sum(r_NK[:,k] * self.log_pi_K[k])
            second = np.sum(r_NK[:,k] * np.sum(stats.norm.logpdf(x_ND, self.mu_KD[k], self.stddev_KD[k]), axis=1))
            third = np.sum(r_NK[:,k] * np.log(r_NK[:,k]))
            tmp = first + second - third
            EM_loss += tmp
        #-1.0 * (log_prior + log_lik + entropy) + penalty_stddev
        return -np.sum(EM_loss) + self.calc_penalty_stddev()
    ...
    def estep__calc_r_NK(self, x_ND):
        ...
        # denominator = 0
        # #pi_K = np.exp(self.log_pi_K)
        # for k in range(self.K):
        #     denominator += logsumexp(self.log_pi_K[k] + stats.norm.logpdf(x_ND, self.mu_KD[k], self.stddev_KD[k]))
        # for k in range(self.K):
        #     r_NK[:, k] = logsumexp(self.log_pi_K[k] + stats.norm.logpdf(x_ND, self.mu_KD[k], self.stddev_KD[k])) / denominator
        # return r_NK
    ...
    def mstep__update_log_pi_K(self, r_NK):
        ...
        N = r_NK.shape[0]
        for k in range(self.K):
            log_pi_K[k] = np.log((r_NK[:,k].sum(axis=0))/N)
        return log_pi_K
    ...
    def mstep__update_mu_KD(self, r_NK, x_ND):
        ...
        for k in range(self.K):
            numerator = np.matmul(r_NK[:,k], x_ND)
            denominator = r_NK[:,k].sum(axis=0)
            mu_KD[k] = numerator/denominator
        return mu_KD
    ...
    def mstep__update_stddev_KD(self, r_NK, x_ND):
        ...
        for k in range(self.K):
            denominator = r_NK[:,k].sum(axis=0) + 1/(np.square(self.variance_penalty_mode) * self.variance_penalty_spread)
            numerator = np.matmul(r_NK[:, k], np.square((x_ND - self.mu_KD[k]))) + 1/(self.variance_penalty_mode * self.variance_penalty_spread)
            stddev_KD[k] = np.sqrt(numerator/denominator)
        return stddev_KD
    ...
    def fit(self, x_ND, x_valid_ND=None, verbose=True):
        ...
        for iter_id in range(self.max_iter + 1):
            ...
            if x_valid_ND is None:
                ...
                va_neg_log_lik_per_pixel = calc_negative_log_likelihood(x_valid_ND,self.log_pi_K, self.mu_KD, self.stddev_KD)
            ...
            if self.do_double_check_correctness:
                ...
                loss_incomplete = va_neg_log_lik_per_pixel
