...
def run_forward_algorithm(log_pi_K, log_A_KK, log_lik_TK):
	...
	* quadratic in number of discrete hidden states K
	...
	numerator =np.multiply(np.exp(log_pi_K) , np.exp(log_lik_TK[0]))
	log_alpha_TK_numerator=np.log(numerator)
	...
	for t in range(1, T):
		# TODO numerator of recursive update for t=1, ... T-1
		...
		numerator=0.0
		numerator=np.multiply(np.sum(np.multiply(np.exp(log_alpha_TK[t-1][:, np.newaxis]),np.exp(log_A_KK)),axis=0),np.exp(log_lik_TK[t]))
		log_alpha_TK_t_numerator = np.log(numerator)
